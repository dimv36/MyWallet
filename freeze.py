...
from mywallet.version import MY_WALLET_VERSION_STR
import shutil
import os
import sys
import glob
...
executables = [Executable('mywallet/__main__.py',
                          icon='./wallet.ico',
                          targetName='mywallet.exe',
                          base='Win32GUI')]
...
bin_path_items = {
    ...
    'mywallet': {
        'excludes': [
                        ('ui', ('addsourcesdialog.ui', 'changemonthbalancedialog.ui', 'mainwindow.ui',
                                'newwalletdialog.ui', 'payoffdebtdialog.ui', 'savingstoincomingdialog.ui',
                                'settingsdialog.ui', 'statisticdialog.ui', 'tablewidget.ui',
                                 'walletdatarangedialog.ui')),
...
class my_build_exe(build_exe_orig):
    def run(self):
        if os.path.exists(self.build_exe):
            shutil.rmtree(self.build_exe)
        self.pre_processing()
        super(my_build_exe, self).run()
    ...
    def post_processing(self):
        ...
        # mywallet-bat
        with open(os.path.join(self.build_exe, 'mywallet-run.bat'), mode='w') as fp:
